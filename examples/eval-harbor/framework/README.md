...
| `T` | `downstream-task` | Answer a downstream task from retained memory; no raw docs are revealed |
...
flowchart LR
  ...
  Agent -->|"update"| Memory
  Stage -->|"T: close-book task only"| Agent
...
`T` stages expose task files but not the source docs. Dataset adapters may split
a public `T` into internal verifier-safe task-family steps. For DynamicMem, the
state and personalized-service task families are validated separately before the
stage server can advance, but the public workflow remains `U/T`.
...
- staged payload with ordered public `U/T` stages;
